...
define build
	...
	@if $(GO) build -o $(BIN_DIR)/$(BIN_VER)/$(BINARY_NAME_ZMAP)-$(1)-$(2) ./Src/ollama-scanner-zmap.go; then \
		echo "Success: $(BINARY_NAME_ZMAP)-$(1)-$(2)"; \
	else \
		echo "Failed: $(BINARY_NAME_ZMAP)-$(1)-$(2)"; \
		exit 1; \
	fi
	@if $(GO) build -o $(BIN_DIR)/$(BIN_VER)/$(BINARY_NAME_MASSCAN)-$(1)-$(2) ./Src/ollama-scanner-masscan.go; then \
